...
async def test_with_keep_alive_dc__no_interval(undine_settings) -> None:
    ...
    async def source() -> AsyncIterator[NextEventDC | CompletedEventDC]:  # noqa: RUF029
...
def _make_session_store() -> SSESessionStore:
    ...
    session.keys = session._session_cache.keys
    session.__setitem__ = lambda _self, key, value: session._session_cache.__setitem__(key, value)
...
def _make_handler() -> tuple[GraphQLOverSSESCHandler, MagicMock, SSESessionStore]:
    ...
    session_mock.__contains__ = lambda _self, key: key in cache
    session_mock.keys = cache.keys
    session_mock.__setitem__ = lambda _self, key, value: cache.__setitem__(key, value)
...
async def test_handler__reserve_stream__no_existing(undine_settings) -> None:
    handler, _signaler, _ = _make_handler()
...
async def test_handler__open_event_stream__stream_not_found(undine_settings) -> None:
    handler, _, _session_store = _make_handler()
...
async def test_handler__execute_operation__cancelled(undine_settings) -> None:
    ...
    async def raises_cancelled(*args, **kwargs) -> AsyncIterator[NextEventSC]:  # noqa: RUF029
