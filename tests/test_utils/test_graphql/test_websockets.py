...
class MockWebSocket:
    ...
    async def close(self, code: GraphQLWebSocketCloseCode | None = None, reason: str | None = None) -> None:
        self.close_code = code
        self.close_reason = reason


async def test_websocket_handler__connect__init_timeout(undine_settings) -> None:
    undine_settings.WEBSOCKET_CONNECTION_INIT_TIMEOUT_SECONDS = 0.001
...
async def test_websocket_handler__receive__subscribe__already_completed(undine_settings) -> None:
    ...
    operation = handler.operations.pop("1")
    assert operation is not None
