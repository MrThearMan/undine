...
def test_convert_filter_ref_to_filter_resolver__model_field():
    ...
    assert result(root=None, info=MockGQLInfo(), value="foo") == models.Q(name__exact="foo")


def test_convert_filter_ref_to_filter_resolver__q_expression():
    q = models.Q(name__in=("foo", "bar"))
...
def test_convert_filter_ref_to_filter_resolver__expression():
    ...
    assert result(root=None, info=MockGQLInfo(), value=True) == models.Q(is_now__exact=True)
...
def test_convert_filter_ref_to_filter_resolver__subquery():
    ...
    assert result(root=None, info=MockGQLInfo(), value=1) == models.Q(first_task__exact=1)
