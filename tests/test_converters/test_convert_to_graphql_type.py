...
def test_convert_to_graphql_type__async_generator__no_args() -> None:
    msg = f"Cannot convert {AsyncGenerator!r} to GraphQL type without generic type arguments."
...
def test_convert_to_graphql_type__future__no_args() -> None:
    msg = f"Cannot convert {asyncio.Future!r} to GraphQL type without generic type arguments."
    with pytest.raises(FunctionDispatcherError, match=exact(msg)):
...
def test_convert_to_graphql_type__maybe_many_or_non_null__many__value_already_non_null__not_stripped() -> None:
    # nullable=False means the NonNull is NOT stripped
