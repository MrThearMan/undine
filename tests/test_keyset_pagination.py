...
from tests.helpers import exact, keyset_cursor
...
from undine.relay import Connection, CursorPaginationHandler, cursor_to_values, encode_base64, values_to_cursor
...
def test_pagination__cursor__value_of_wrong_type(graphql, undine_settings) -> None:
    ...
    cursor = values_to_cursor("TaskType", ["not-an-integer"])
...
def test_pagination__cursor__cursor_payload_is_not_a_list() -> None:
    cursor = encode_base64('connection:TaskType:{"foo": 1}')

    with pytest.raises(ValueError, match=exact("Cursor does not contain a list of ordering values.")):
        cursor_to_values("TaskType", cursor)


def test_pagination__cursor__cursor_for_another_type() -> None:
    cursor = values_to_cursor("TaskType", ["1"])

    with pytest.raises(ValueError, match=exact("Cursor was not created for type 'PersonType'.")):
        cursor_to_values("PersonType", cursor)
