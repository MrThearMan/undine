...
from undine.errors.exceptions import (
    ConnectionQueryTypeNotNodeError,
    EmptyFilterResult,
    ...
    GraphQLBulkMutationObjectNotFoundError,
...
def test_error__connection_query_type_not_node_error():
    error = ConnectionQueryTypeNotNodeError(query_type=MyClass)

    assert error.args[0] == (
        "QueryType 'tests.test_errors.test_exceptions.MyClass' does not implement the Node interface"
    )


def test_error__empty_filter_result():
    error = EmptyFilterResult()

    assert error.args[0] == "Filter result should be null."
...
def test_error__model_field_not_a_relation_error():
...
def test_error__graphql_bulk_mutation_object_not_found_error():
    error = GraphQLBulkMutationObjectNotFoundError(model=Task, value=1)

    assert error.message == (
        "'example_project.app.models.Task' object with the primary key 1 does not exist. "
...
def test_error__graphql_node_id_field_type_error():
    error = GraphQLNodeIDFieldTypeError(typename="foo")

    assert error.message == (
        "The 'id' field of the object type 'foo' must be of type 'ID' to comply with the 'Node' interface."
    )
    assert error.extensions == {"error_code": "NODE_ID_NOT_GLOBAL_ID", "status_code": 400}


def test_error__graphql_node_interface_missing_error():
    error = GraphQLNodeInterfaceMissingError(typename="foo")

    assert error.message == "Object type 'foo' must implement the 'Node' interface."
    assert error.extensions == {"error_code": "NODE_INTERFACE_MISSING", "status_code": 400}


def test_error__graph_ql_node_invalid_global_id_error():
    error = GraphQLNodeInvalidGlobalIDError(value="foo")

    assert error.message == "'foo' is not a valid Global ID."
    assert error.extensions == {"error_code": "NODE_INVALID_GLOBAL_ID", "status_code": 400}


def test_error__graphql_node_missing_id_field_error():
    error = GraphQLNodeMissingIDFieldError(typename="foo")

    assert error.message == "The object type 'foo' doesn't have an 'id' field."
    assert error.extensions == {"error_code": "NODE_QUERY_TYPE_ID_FIELD_MISSING", "status_code": 400}


def test_error__graphql_node_object_type_missing_error():
    error = GraphQLNodeObjectTypeMissingError(typename="foo")

    assert error.message == "Object type 'foo' does not exist in schema."
    assert error.extensions == {"error_code": "NODE_MISSING_OBJECT_TYPE", "status_code": 400}


def test_error__graphql_node_query_type_missing_error():
    error = GraphQLNodeQueryTypeMissingError(typename="foo")

    assert error.message == "Cannot find undine QueryType from object type 'foo'."
    assert error.extensions == {"error_code": "NODE_QUERY_TYPE_MISSING", "status_code": 400}
