...
from typing import TYPE_CHECKING, Any, AsyncIterator, Iterable, Literal, NotRequired, Protocol, TypedDict, Unpack
...
from graphql import FormattedExecutionResult, GraphQLSchema
...
from undine.typing import GQLInfo, RequestMethod, SSEState
...
class SSEStream:
    ...
    # `ApplicationCommunicator.__del__` cancels the task running the consumer, so a communicator
    # that is only referenced by the call that created it takes its operation down with it.
    # Keeping them here ties their lifetime to the stream, and thus to the test.
    operation_communicators: list[ApplicationCommunicator] = dataclasses.field(default_factory=list)


async def open_sse_stream() -> SSEStream:
    """Create a user and a session, reserve a stream for them, and open it."""
    ...
    return SSEStream(user=user, session=session, token=token, communicator=communicator)


async def submit_sse_operation(
    stream: SSEStream,
    *,
    query: str,
    operation_id: str,
    ...
    """Submit an operation for a stream with a POST request and check that it was accepted."""
    payload: dict[str, Any] = {"query": query, "extensions": {"operationId": operation_id}}
    if variables is not None:
        payload["variables"] = variables

    communicator = make_sse_operation_communicator(user=stream.user, session=stream.session, token=stream.token)
    stream.operation_communicators.append(communicator)

    await sse_send_request(communicator, body=json.dumps(payload).encode())
    response = await sse_get_response(communicator)

    assert response["status"] == HTTPStatus.ACCEPTED, f"{response=}"


async def cancel_sse_operation(stream: SSEStream, *, operation_id: str) -> None:
    """Cancel an operation on a stream with a DELETE request and check that it was cancelled."""
    ...
    assert response["status"] == HTTPStatus.OK, f"{response=}"


async def read_sse_next_event(stream: SSEStream, *, operation_id: str) -> FormattedExecutionResult:
    """Read a "next" event for the given operation from the stream and return its payload."""
    event = await sse_read_stream_event(stream.communicator)

    assert event["event"] == "next", f"{event=}"
    assert event["data"]["id"] == operation_id, f"{event=}"

    return event["data"]["payload"]


async def read_sse_complete_event(stream: SSEStream, *, operation_id: str) -> None:
    """Read the "complete" event that ends the given operation from the stream."""
    event = await sse_read_stream_event(stream.communicator)

    assert event["event"] == "complete", f"{event=}"
    assert event["data"]["id"] == operation_id, f"{event=}"


async def disconnect_sse_stream(stream: SSEStream) -> None:
    """Drop the stream connection and let the consumer run its cleanup."""
    await stream.communicator.send_input({"type": "http.disconnect"})
    await asyncio.sleep(TEST_WAIT_TIME)


def create_query_schema() -> GraphQLSchema:
    """Schema with a single `test` query."""
...
def create_mutation_schema() -> GraphQLSchema:
    """Schema with a single `doSomething` mutation."""
...
def create_slow_subscription_schema() -> GraphQLSchema:
    """Schema whose `slow` subscription yields one value and then hangs, so the operation stays running."""
    ...
    return create_schema(query=Query, subscription=Subscription)


def create_failing_subscription_schema(error: Exception) -> GraphQLSchema:
    """Schema whose `errorStream` subscription yields one value and then raises the given error."""
