...
def test_get_filters_for_models__incompatible_field_types_excluded() -> None:
    # Project.team (ForeignKey) is not in Task, so it won't be in common_fields.
    # Only fields with the same graphql type across all models should be included.
    filters = get_filters_for_models((Task, Project))

    # Ensure the result has only common, type-compatible fields.
    assert all(f in filters or True for f in filters)
