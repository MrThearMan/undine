...
@pytest.mark.django_db
def test_pagination_handler__paginate_queryset__entrypoint_limit_not_applied(graphql, undine_settings) -> None:
    ...
    class Query(RootType):
        tasks = Entrypoint(OffsetPagination(TaskType), limit=1)
    ...
    assert response.data == {"tasks": [{"name": "Task 2"}, {"name": "Task 3"}]}
...
@pytest.mark.django_db
def test_pagination_handler__paginate_prefetch_queryset__reverse_foreign_key(graphql, undine_settings) -> None:
    """Offset pagination of a nested reverse foreign key, which needs no prefetch hack."""
    ...
    class TaskType(QueryType[Task], auto=False):
        name = Field()
        steps = Field(OffsetPagination(TaskStepType, page_size=2))
    ...
    assert response.data == {"tasks": [{"name": "Task 1", "steps": [{"name": "a"}, {"name": "b"}]}]}
