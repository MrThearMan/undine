...
class GraphQLParamsParser:
    ...
    def parse_body(cls, request: HttpRequest) -> dict[str, str]:
        ...
        content_type = MediaType(request.content_type or "application/json; charset=utf-8")
        charset: str = content_type.params.get("charset", "utf-8")

        if content_type.main_type == "application" and content_type.sub_type == "graphql":
            return {"query": cls.decode_body(request.body, charset=charset)}

        if content_type.main_type == "application" and content_type.sub_type == "json":
            return cls.parse_json_body(request.body, charset=charset)

        if content_type.main_type == "multipart" and content_type.sub_type == "form-data" and request.FILES:
            return cls.parse_file_uploads(request.POST.dict(), request.FILES.dict())

        if (  #
            (content_type.main_type == "application" and content_type.sub_type == "x-www-form-urlencoded")
            or (content_type.main_type == "multipart" and content_type.sub_type == "form-data")
        ):
    ...
    def decode_body(cls, body: bytes, charset: str = "utf-8") -> str:
        try:
            ...
            raise GraphQLError(message=msg, original_error=error) from error
    ...
    def load_json_dict(cls, string: str, *, decode_error_message: str, type_error_message: str) -> dict[str, Any]:
        try:
            ...
            raise GraphQLError(message=decode_error_message, original_error=error) from error

        if not isinstance(data, dict):
            raise GraphQLError(message=type_error_message) from None
        return data
    ...
    def get_map(cls, post_data: dict[str, str]) -> dict[str, list[str]]:  # pragma: no cover
        ...
        for value in files_map.values():
            if not isinstance(value, list) or not all(isinstance(item, str) for item in value):
                ...
                raise GraphQLError(message=msg) from None
    ...
    def get_graphql_params(cls, data: dict[str, str]) -> GraphQLParams:
        ...
        return GraphQLParams(query=query, variables=variables, operation_name=operation_name, extensions=extensions)
