...
class OrderSetMeta(type):
    ...
    def __call__(cls, ref: T) -> T:
        ...
        if is_subclass(ref, QueryType):
            ...
            msg = f"Cannot add an OrderSet to a {ref.__class__.__name__}"
    ...
    def __add_to_query_type__(cls, query_type: type[QueryType]) -> None:
        ...
        if len(models) != 1:
            msg = "Cannot add an OrderSet with multiple models to a QueryType"
            raise TypeError(msg)  # TODO: Custom error
    ...
    def __add_to_union_type__(cls, union_type: type[UnionType]) -> None:
        models = cls.__models__
        if len(models) == 1:
            msg = "Cannot add an OrderSet with a single models to a UnionType"
            raise TypeError(msg)  # TODO: Custom error
        ...
        if on_union_type != on_orderset:
            msg = "Cannot add an OrderSet to a UnionType with different models"
            raise TypeError(msg)  # TODO: Custom error
