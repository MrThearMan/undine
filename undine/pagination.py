...
def _add_start_index(queryset: QuerySet, start: int) -> QuerySet:
    """Add an annotation to the given queryset with the start index of the current page."""
    return queryset.annotate(**{undine_settings.PAGINATION_START_INDEX_KEY: Value(start)})
...
def _add_stop_index(queryset: QuerySet, stop: int) -> QuerySet:
    """Add an annotation to the given queryset with the stop index of the current page."""
    return queryset.annotate(**{undine_settings.PAGINATION_STOP_INDEX_KEY: Value(stop)})
