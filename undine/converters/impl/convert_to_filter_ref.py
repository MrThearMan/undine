...
def _(ref: Q, **kwargs: Any) -> Any:
    ...
    def aliases(self: Filter, info: GQLInfo) -> dict[str, DjangoExpression]:
        results: dict[str, DjangoExpression] = {}
        if user_func is not None:
            results |= user_func(self, info)

        results[self.name] = ref
        return results
...
with suppress(ImportError):
    ...
    def _(ref: PostgresFTS, **kwargs: Any) -> Any:
        ...
        def aliases(ftr: Filter, info: GQLInfo) -> dict[str, DjangoExpression]:
            results: dict[str, DjangoExpression] = {}
            if user_func is not None:
                results |= user_func(ftr, info)

            lang = ref.get_search_language(info)
            key = ref.get_vector_alias_key(ftr, lang)
