...
class BulkMutationHandler(Generic[TModel]):
    ...
    def create_many(
        ...
        *,
        ...
        """Create a new instance of the model, while also handling model relations."""
        # TODO: Refactor this. Support upsert?
    ...
    def update_many(
        ...
        *,
        ...
        for item in input_data:
            instance = instances.get(item[lookup_field])
            if instance is None:
    ...
    def pre_save(self, input_data: list[dict[str, Any]]) -> None:
        """
        Make pre-save modifications to the given input data based on the model's relations.
        Currently only fetches forward one-to-one and many-to-one relations in bulk and
        sets them to the corresponding fields in the input data.
        # TODO: Reverse relations? Creating new instances?
        """
        prefetch: dict[type[Model], set[int]] = defaultdict(set)
        ...
        for item in input_data:
            for field_name, value in item.items():
                ...
                if field_info.relation_type.created_before:
                    # TODO: What if value is not a pk value? Raise error?
                    prefetch[field_info.model].add(value)

                # TODO: How to handle 'created_after' relations? Raise error?

        instances: dict[type[Model], dict[int, Model]] = {}
        for model, ids in prefetch.items():
            instances[model] = {instance.id: instance for instance in model.objects.filter(id__in=ids)}
        ...
        for item in input_data:
            for field_name, value in item.items():
                ...
                if field_info.relation_type.created_before:
                    instance = instances[field_info.model].get(value, None)
                    # TODO: Should raise error here if instance is None?
