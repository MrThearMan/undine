...
class PostgresFTS:
    ...
    def __init__(
        ...
        **lang_specific: Unpack[PostgresFTSLangSpecificFields],
        ...
        self.vectors: dict[FTSLang, SearchVector] = {
            lang: SearchVector(*set(*common, *fields), config=lang)  # Create search vectors in advance
    ...
    def __call__(self, ftr: Filter, info: GQLInfo, *, value: str) -> Q:
        lang = self.get_search_language(info)
        key = self.get_vector_alias_key(ftr, lang)
        search = build_pg_search(value, separator=self.separator)
...
def build_pg_search(text: str, *, separator: LangSep = "|") -> str:
    ...
    search_terms = (f"'{value}':*" for value in text.split(" ") if value)
...
def get_request_search_language(info: GQLInfo) -> SearchLanguage:
    ...
    lang = get_language_from_request(info.context, check_path=True)  # type: ignore[assignment]
    return TextSearchLang.for_code(lang)
