...
from undine.typing import DjangoRequestProtocol, SSEState, SSEStreamState
...
def raised_exceptions_to_responses(func: Callable[P, Awaitable[None]]) -> Callable[P, Awaitable[None]]:
    """Wraps raised exceptions as GraphQL responses if they happen in `execute_graphql_sse_sc`."""
...
class GraphQLOverSSESingleConnectionHandler:
    ...
    async def receive(self, request: DjangoRequestProtocol) -> None:
        """Process aa GraphQL over SSE request"""
    ...
    async def get_event_stream(self, request: DjangoRequestProtocol) -> None:
        ...
        if stream_state["state"] == SSEState.REGISTERED:
            stream_state["state"] = SSEState.OPENED
        elif stream_state["state"] == SSEState.OPENED:
    ...
    async def subscribe(self, request: DjangoRequestProtocol) -> None:
        ...
        await request.session.aset(key=operation_key, value=now)
        await request.session.asave()
        ...
        response = HttpResponse(content="Operation completed", content_type="text/plain", status=HTTPStatus.ACCEPTED)
        await self.send_http_response(response)
    ...
    async def cancel_subscription(self, request: DjangoRequestProtocol) -> None:
        ...
        response = HttpResponse(content="Operation canceled", content_type="text/plain", status=HTTPStatus.OK)
        await self.send_http_response(response)
    ...
    async def send_graphql_response(self, result: ExecutionResult) -> None:
        await self.send_single_response(
    ...
    async def send_graphql_error_response(self, error: GraphQLError | GraphQLErrorGroup) -> None:
        ...
        await self.send_single_response(
            body=json.dumps(result.formatted, separators=(",", ":")),
            status=error.extensions.get("status_code", HTTPStatus.OK),
            headers={"Content-Type": "application/json"},
        )
