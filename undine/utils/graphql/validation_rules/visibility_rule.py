...
if TYPE_CHECKING:
    ...
    from graphql import FieldNode, GraphQLCompositeType, GraphQLEnumType, GraphQLInputObjectType, VisitorAction
...
__all__ = [
    "VisibilityRule",
    "partial_validation_rule",
]


def partial_validation_rule(*, request: DjangoRequestProtocol) -> type[VisibilityRule]:
...
class VisibilityRule(ValidationRule):
    ...
    def enter_argument(self, node: ast.ArgumentNode, *args: Any) -> VisitorAction:  # noqa: PLR0911
        ...
        if undine_filterset is not None:
            if not undine_filterset.__is_visible__(self.request):
                ...
                self.report_argument_error(parent_type, field_node, node)
                return None

            self.handle_filters(graphql_input_type, node.value)  # type: ignore[arg-type]
            return None
        ...
        if undine_orderset is not None:
            ...
            self.handle_orders(undine_orderset, graphql_input_type, node.value)  # type: ignore[arg-type]
            return None
        ...
        if undine_mutation_type is not None:
            ...
            self.handle_inputs(graphql_input_type, node.value)  # type: ignore[arg-type]
            return None
    ...
    def flatten_object_field_nodes(
        self,
