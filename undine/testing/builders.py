    ...
    for key, value in filter_params.items():
        ...
        if len(plain_key.split(LOOKUP_SEP)) > 1:
            ...
            params[to_camel_case(key)] = _format_value_for_filter(
                value,
                is_order_by=key == undine_settings.ORDER_BY_INPUT_TYPE_KEY,
            )
...
def _fields_to_dict(fields: str) -> dict[str, dict | None]:
    ...
    for field_name in (val for val in fields.replace("\n", "").split(" ") if val != ""):
        if field_name == "{":
            ...
            parents.pop()
            current_dict = _get_nested(fields_dict, *parents, default={})
...
def _get_nested(obj: dict | list | None, /, *args: str | int, default: Any = None) -> Any:
    """
    ...
    if isinstance(arg, int):
        ...
        try:
            ...
            obj = None
        return _get_nested(obj, *args, default=default)

    obj = obj or {}
    try:
        return _get_nested(obj.get(arg), *args, default=default)
