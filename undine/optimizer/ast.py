...
from undine.relay import PageInfoType
...
class GraphQLASTWalker:  # noqa: PLR0904
    ...
    def handle_object_type(self, field_type: GraphQLOutputType, field_node: FieldNode) -> None:
        ...
        if is_connection(field_type):
            return self.handle_connection(field_type, field_node)

        if is_edge(field_type):
            return self.handle_edge_field(field_type, field_node)

        if field_type == PageInfoType:
            return self.handle_page_info_field(field_type, field_node)
    ...
    def handle_connection(self, parent_type: GraphQLOutputType, field_node: FieldNode) -> None:
        field_type = self.get_field_type(parent_type, field_node)
        ...
        if field_node.name.value == "totalCount":
            return self.handle_total_count(field_type, field_node)

        return None  # pragma: no cover

    def handle_edge_field(self, parent_type: GraphQLOutputType, field_node: FieldNode) -> None:
        field_type = self.get_field_type(parent_type, field_node)

        if field_node.name.value == "node":
            selections = get_selections(field_node)
            return self.handle_selections(field_type, selections)
    ...
    def get_undine_field(self, field_type: GraphQLOutputType, field_node: FieldNode) -> UndineField | None:
...
def is_edge(field_type: GraphQLOutputType) -> bool:
    return field_type.name.endswith("Edge") and "cursor" in field_type.fields and "node" in field_type.fields
