...
class GraphQLASTWalker:
    ...
    def handle_object_type(self, field_type: GraphQLOutputType, field_node: FieldNode) -> None:
        field_name = camel_case_to_name(field_node.name.value)
    ...
    def get_model(self, field_type: GraphQLOutputType) -> type[Model] | None:
        return getattr(field_type.extensions.get(undine_settings.MODEL_TYPE_EXTENSIONS_KEY), "__model__", None)
    ...
    def get_field_name(self, field_node: FieldNode) -> str:
        ...
        return alias or camel_case_to_name(field_node.name.value)
...
def is_foreign_key_id(field: Field, field_node: FieldNode) -> bool:
    return isinstance(field, ForeignKey) and field.get_attname() == camel_case_to_name(field_node.name.value)
