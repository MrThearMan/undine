...
class MutationTypeMeta(type):
    ...
    # Calculated after inputs are connected
...
class MutationType(Generic[TModel], metaclass=MutationTypeMeta):
    ...
    `kind: Literal["create", "update", "delete", "related"] = <inferred>`
        The kind of mutation this is. Infer used mutation type from mutation name.
    ...
    @classmethod
    def __bulk_mutate__(cls, instance: list[TModel], info: GQLInfo, input_data: list[dict[str, Any]]) -> Any:
        """Override this method for custom bulk mutations."""
