...
class PreProcessingDocstringParser:
    ...
    def parse_body(self, docstring: str | None) -> str:
    ...
    def parse_return_description(self, docstring: str | None) -> str:
        if docstring is None:
            return ""
        docstring = cleandoc(docstring).strip()
...
class RSTDocstringParser:
    ...
    def parse_arg_descriptions(cls, docstring: str) -> dict[str, str]:
        ...
        return {name: re.sub(cls.TRIM_PATTERN, " ", text) for name, text in found_args}
