...
@cache
def parse_class_variable_docstrings(cls: type, /) -> dict[str, str]:
    """
    Parse class variable docstrings from the given class.
    ...
    ...     '''Another description.'''
    ...
    >>> parse_class_variable_docstrings(Foo)
    ...
    for expr in class_def.body:
        ...
        if name is not None:
            if isinstance(expr, ast.Expr):
                if isinstance(expr.value, ast.Constant):
                    ...
                    descriptions[name] = inspect.cleandoc(expr.value.id)
            name = None
            continue
        ...
        if isinstance(expr, ast.AnnAssign):
            ...
            name = expr.targets[0].id  # type: ignore[attr-defined]
