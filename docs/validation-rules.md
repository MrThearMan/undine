...
When a GraphQL operation consists of three main parts: parsing, validation, and execution.
...
Usually this happens when defining fragments. Named types include scalars, object types, interfaces,
unions, enums, input objects, and directives.
...
A `ValidationRule` instance has access to the `ValidationContext` instance, through
which you can access to commonly useful contextual information from within a validation rule.
